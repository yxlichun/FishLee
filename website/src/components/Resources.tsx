  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">资源收藏</h1>
          <p className="text-gray-500 mt-2">整理学习资源和工具</p>
        </div>
        {!isAdding && (
      ...
      {isAdding && (
        <div className="card mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-gray-900">添加资源</h2>
      ...
      {bookmarks.length === 0 ? (
        ...
        <div className="space-y-8">
          {Object.entries(groupedBookmarks).map(([category, items]) => {
            ...
            return (
              <div key={category}>
                <h2 className="text-xl font-semibold text-gray-900 mb-4">{categoryLabel}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {items.map((bookmark) => (
                    <div key={bookmark.id} className="card hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-2">
                        <a
                          href={bookmark.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 text-lg font-medium text-brand-600 hover:text-brand-700 flex items-center gap-2"
                        >
                          {bookmark.title}
